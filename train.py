...
def train(opt):
    ...
    if opt.model_reload is None:
        ...
        print(
            "Continue existing model, from epoch %d, batch %d to epoch %d"
    ...
    for epoch_idx in range(model_start_epoch, opt.epoch):
        ...
        for batch_idx in range(start_batch, train_dataset.num_batch):
            ...
            if (1 + batch_idx) % opt.save_freq == 0:
                ...
                model.eval()
                val_loss = evaluate(
                    model=model, eval_dataset=val_dataset, cuda=opt.cuda
                )
                model.train()
    ...
    print("Best model:")
    print(
        "Epoch : %d, Batch : %d ,Loss : %f, Perplexity : %f"
        % (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--val_data",
        type=str,
        default="./data/penn/valid.txt",
        help="Validation data path",
    )
    parser.add_argument("--model_name", type=str, default="penn-lm", help="Model name")
    parser.add_argument("--model_reload", type=str, default=None, help="Relaod model")
    parser.add_argument(
        "--dim_word", type=int, default=256, help="Dimension of word embeddig vector"
    )
    parser.add_argument("--dim_rnn", type=int, default=512, help="Dimension of LSTM")
    parser.add_argument(
    ...
    parser.add_argument(
        "--epoch", type=int, default=10, help="Finish after several epochs"
    )
    ...
    parser.add_argument(
        "--lr_decay", type=float, default=1.0, help="Learning rate decay every epoch"
    )
    parser.add_argument(
        "--clip", type=float, default=5, help="Prevent gradient explode"
    )
    parser.add_argument("--dropout_rate", type=float, default=0.3, help="Dropout rate")
    parser.add_argument(
        "--display_freq", type=int, default=10, help="Display every several bathces"
    )
    parser.add_argument(
        "--save_freq", type=int, default=10, help="Save model every several epoch"
    )
    parser.add_argument(
        "--random_seed", type=int, default=111, help="Random seed to reproduce result"
    )
