...
def evaluate(model=None, eval_dataset=None, cuda=True):
    ...
    if cuda:
        criterion = nn.CrossEntropyLoss(
            weight=criterion_weight, size_average=False
        ).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=64, help="Evaluate batch size if use gpu"
    )
