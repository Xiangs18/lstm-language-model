...
class DataSet:
    ...
    def build_dict(self, save_as_text=True):
        ...
        with open(self.datapath, "r") as f:
            ...
            self.dictionary = OrderedDict(
                zip(self.frequency.keys(), range(1, self.num_vocb + 1))
            )
    ...
    def index_token(self):
            ...
            for count, line in enumerate(f):
                ...
                if len(tokens) == 0:
                    ...
                    sequence = [
                        ...
                        else self.dictionary[const.UNK_WORD]
                        for token in tokens
                    ]
        ...
        self.index = range(self.num_batch)
        print(
            "%d sentences were processed, %d longer than maximum length,%d were ignored because zero length"
            % (len(self.sentence), long_sentence, zero_sentence)
        )
        self.describe_dataset()
        print("Done.")
    ...
    def get_batch(self, batch_idx):
        ...
        for i in range(self.batch_size):
            ...
            batch_data[: len_ - 1, i].copy_(
                torch.LongTensor(self.sentence[sequence_idx][: len_ - 1])
            )
            target_words[: len_ - 1, i].copy_(
                torch.LongTensor(self.sentence[sequence_idx][1:len_])
            )
